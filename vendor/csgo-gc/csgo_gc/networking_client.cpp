...
bool NetworkingClient::HandleMessage(ClientGC *gc, uint64_t steamId, GCMessageRead &message)
{
    ...
    if (typeUnmasked == k_EMsgNetworkRequestSOCache)
    {
        const auto now = std::chrono::steady_clock::now();
        if (m_serverSteamId && steamId == m_serverSteamId && !message.RemainingBytes()
            && now - m_lastSOCacheRequest >= std::chrono::seconds{ 1 })
        {
            m_lastSOCacheRequest = now;
