...
void SharedGC::StartThread(bool periodicWork)
{
    ...
    {
        {
            ...
            const auto ready = [this] { return !m_gcEvents.empty() || m_stopping; };
            if (m_periodicWork)
                m_cv.wait_for(lock, std::chrono::milliseconds{ 100 }, ready);
            else
        ...
        events.clear();
        if (m_periodicWork) HandlePeriodicWork();
    }
}
...
void SharedGC::PostToGC(GCEvent type, uint64_t id, const void *data, uint32_t dataSize)
{
    ...
    event.id = id;
    if (dataSize) event.buffer.assign(dataBegin, dataBegin + dataSize);
